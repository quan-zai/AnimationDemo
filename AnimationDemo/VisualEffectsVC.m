...
- (void)layerVisualEffects
{
     ...
     ****************************************************
     ...
     CPU 计算好显示内容提交到 GPU，GPU 渲染完成后将渲染结果放入帧缓冲区，随后视频控制器会按照 VSync 信号逐行读取帧缓冲区的数据，经过可能的数模转换传递给显示器显示。（和电视机的显示机制类似）
     ...
     注意：
     对于经常变动的内容,这个时候不要开启,否则会造成性能的浪费 （也就说对于底部图层或者说很少需要重绘的图层，开启光栅化是可以相应的提高性能的）
